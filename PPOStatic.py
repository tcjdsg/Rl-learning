...
from models.mb_agg import g_pool_cal
...
def main(epochs):
    ...
    # train_dataset = JZJ(configs.n_j, configs.n_m, configs.low, configs.high, configs.num_ins, 200)
    # validat_dataset = JZJ(configs.n_j, configs.n_m, configs.low, configs.high, 120, 200)
    ...
    for epoch in range(epochs):
        ...
        for i in range(1, configs.max_iterations + 1):
            ...
            if (batch_idx + 1) % step  == 0 :
                ...
                filename = 'FJSP_1{}'.format('J'+str(configs.n_j)+'M'+str(configs.n_m))
                filepath = os.path.join(filepath, filename)
                epoch_dir = os.path.join(filepath, '%s_%s' % (100, batch_idx))
                if not os.path.exists(epoch_dir):
                    os.makedirs(epoch_dir)
                job_savePath = os.path.join(epoch_dir, '1{}.pth'.format('policy_job'))
                machine_savePate = os.path.join(epoch_dir, '1{}.pth'.format('policy_mch'))
